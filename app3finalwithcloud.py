...
async def get_resume_and_job_description(
    resume_text: str = Query(..., title="Resume Text"),
    job_description: str = Query(..., title="Job Description")
):
    try:
        # Fetch resume text from URL
        resume_text = extract_text_from_s3_pdf(resume_text)
        job_desc_text = extract_text_from_s3_pdf(job_description)
        # print("@@@@@@")
        # print(resume_text)
        print("@@@@@@")
        print(job_desc_text)
